...
export const productsDataSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSelectedBrand: (state: ProductsDataState, action: PayloadAction<null | number>) => {
      state.selectedBrand = action.payload
    },
    updateProducts: (state: ProductsDataState) => {
      state.products = state.selectedBrand === null
        ? [...state.data]
        : [...state.data].filter((item: IProductItem) => item.brand === state.selectedBrand)
      state.products = getSortedProducts(state.products, state.sortedValue)
    }
  },
  extraReducers: (builder) => {
    builder.addCase(fetchProductsItemsData.fulfilled, (state, action: PayloadAction<Array<IProductItem>>) => {
...
export const { setSelectedBrand, updateProducts } = productsDataSlice.actions
